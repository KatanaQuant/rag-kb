...
class SQLiteSource:
    ...
    def get_vectors(self) -> List[Tuple]:
        """Get all vector embeddings using knn_search

        vectorlite requires knn_search to query - direct SELECT doesn't work.
        We use a zero vector with high top_k to enumerate all embeddings.

        Returns list of (rowid, embedding) tuples.
        """
        try:
            # vectorlite requires knn_search - direct SELECT rowid doesn't work
            # Use zero vector with max_elements top_k to get all vectors
            zero_vec = b'\x00' * (1024 * 4)  # 1024 dim * 4 bytes/float
            max_k = 200000  # Must match max_elements in table definition
            ...
            for rowid, emb in cursor:
                if isinstance(emb, bytes):
                    ...
                    embedding = list(emb)
                results.append((rowid, embedding))

            print(f"Read {len(results)} vectors via knn_search")
            return results
