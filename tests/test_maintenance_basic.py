...
"""Tests for basic maintenance REST API endpoints
...
NOTE: These tests use SQLite databases for testing. The maintenance routes
use OperationsFactory which selects the right backend implementation.
"""
import pytest
...
class TestBackfillChunkCountsEndpoint:
    ...
    def test_backfill_chunk_counts_dry_run(self, client):
        ...
        with patch.dict('sys.modules', {'migrations.backfill_chunk_counts': mock_module}):
            ...
            # Endpoint must return success - 500 indicates a bug
...
class TestDeleteEmptyDocumentsEndpoint:
    ...
    def test_delete_empty_documents_finds_orphans(self, client, temp_db):
        ...
        mock_cursor.fetchall.return_value = [(1, "/test/orphan.pdf")]
        mock_db.connect.return_value.cursor.return_value.__enter__ = MagicMock(return_value=mock_cursor)
        mock_db.connect.return_value.cursor.return_value.__exit__ = MagicMock(return_value=None)

        with patch('routes.maintenance.DatabaseFactory') as MockFactory:
            MockFactory.create_connection.return_value = mock_db
    ...
    def test_delete_empty_documents_skips_documents_with_chunks(self, client, temp_db):
        ...
        mock_db = MagicMock()
        mock_cursor = MagicMock()
        # No orphans found - documents have chunks
